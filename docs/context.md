...
| `dataengine` | PostgreSQL+PostGIS detras de PgBouncer. GeoServer se conecta a `dataengine-primary:5432` via red docker `dataengine-network`. El indice espacial de `economia.cultivos`, su columna `geom_3857` (reproyeccion materializada) y `clave_municipio` (spatial join para filtro por municipio) los provisiona dataengine via migraciones Alembic (`0020`–`0023`), no GeoServer. La capa se republica nativa en EPSG:3857 con `scripts/init-cultivos-layer.sh` (la vista SQL expone `geom_3857` + `clave_municipio`). |
